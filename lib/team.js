...
function mockMember({ memberId, nickname, avatarSeed }) {
  const ts = Date.now();
  return {
...
export class LocalTeamMock {
  ...
  async createTeam({ founder } = {}) {
    ...
    const team = {
      ...
      members: [mockMember({ memberId, nickname: f.nickname, avatarSeed: f.avatarSeed })],
      recentPokes: [],
      version: 1,
      local: true,
    };
  ...
  async joinTeam({ code, member } = {}) {
    ...
    if (idx >= 0) {
      team.members[idx] = { ...team.members[idx], nickname: String(m.nickname || team.members[idx].nickname), avatarSeed: m.avatarSeed ? String(m.avatarSeed) : team.members[idx].avatarSeed, lastSeenAt: Date.now() };
    } else {
      team.members.push(mockMember({ memberId, nickname: m.nickname, avatarSeed: m.avatarSeed }));
    }
  ...
  async heartbeat({ code, memberId, snapshot } = {}) {
    const team = this._get(code);
    if (!team) return { ok: false, error: 'not_found', team: null };
