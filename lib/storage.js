// lib/storage.js — v0.3.3: 多任务 + 用户资料 + 可选同步 + 备份基础
const KEYS = {
...
export const Storage = {
  ...
  async getTasks() {
    ...
    return (await resolveValue(KEYS.TASKS, Array.isArray(localValue) ? localValue : [])) || [];
  },
  ...
  async saveTask(task) {
    ...
    const idx = list.findIndex((item) => sameId(item?.id, task?.id));
  ...
  async getBackupData() {
    const [settings, tasks, history, stats, profile, syncEnabled, recentBreakdowns] = await Promise.all([
      this.getSettings(),
      this.getTasks(),
      this.getHistory(),
      this.getStats(),
    ...
    return { settings, tasks, history, stats, profile, syncEnabled, recentBreakdowns };
  },
