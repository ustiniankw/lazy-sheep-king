...
// ---------------------------------------------------------------------------
// v0.8.0 · 云同步 vault 派生
//  - vaultId：用户在新设备只输入短语即可稳定重算，用于「找到」自己的 blob
//  - vaultToken：独立密钥用于 Worker 鉴权，服务端只看到 hash 后的 token，不接触短语
// 两者都取 SHA-256 的前 32 个 hex 字符（128bit），足够唯一且不可逆推短语。
// ---------------------------------------------------------------------------
...
export async function deriveVaultId(mnemonic) {
  const hex = await sha256Hex(`${normalizeMnemonic(mnemonic)}vault-v1`);
  return hex.slice(0, 32);
}

export async function deriveVaultToken(mnemonic) {
  const hex = await sha256Hex(`${normalizeMnemonic(mnemonic)}auth-v1`);
  return hex.slice(0, 32);
}
