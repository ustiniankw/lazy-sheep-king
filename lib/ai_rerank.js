...
export async function detectAvailableTier(settings = {}) {
  // FIX 4（v0.5.1）：优先使用用户自配 API（更稳定、可控），
  // Chrome 内置 Prompt API 因浏览器兼容不稳定，降级为机会性 Tier 2。
  const hasUserApi = !!(settings?.llm?.enabled && settings?.llm?.apiKey && settings?.llm?.baseUrl);
  if (hasUserApi) return 'user-api';
  if (hasChromeLanguageModel()) return 'chrome-ai';
  return 'none';
}

// 供诊断面板 / options 使用：Chrome 内置 Prompt API 是否可用（被动探测，不触发下载）
export function chromePromptApiAvailable() {
  return hasChromeLanguageModel();
}
