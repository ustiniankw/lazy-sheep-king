...
export async function breakdownTask(goal, settings) {
  if (settings?.llm?.enabled && settings?.llm?.apiKey) {
    try {
      const result = await callLLMForBreakdown(goal, settings);
      return { source: 'llm', ...result };
    } catch (error) {
      const local = await localBreakdown(goal);
      return { source: 'fallback', warning: error.message || String(error), steps: local.steps, meta: local.meta };
    }
  }
  const local = await localBreakdown(goal);
  return { source: 'fallback', steps: local.steps, meta: local.meta };
}
